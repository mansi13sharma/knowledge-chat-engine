...
- `backend/app/api/routes/admin_knowledge_base.py` — the `/api/admin/knowledge-base` router: upload (`POST /documents`), list (`GET /documents`), delete (`DELETE /documents/{id}`), re-index (`POST /documents/{id}/reindex`), stats (`GET /stats`). Every route requires an admin session (see below) — enforced router-wide via a `Depends(require_admin)` on the `APIRouter` itself, not per-route.
- `backend/app/services/auth.py`, `backend/app/api/routes/admin_auth.py` — single hardcoded admin email/password account (`ADMIN_EMAIL`/`ADMIN_PASSWORD_HASH`/`ADMIN_AUTH_SECRET`) gating all `/api/admin/*` routes. `POST /api/admin/auth/login` issues an HMAC-signed `{email, exp}` session token (`ADMIN_SESSION_TTL_SECONDS`, default 12h); `require_admin` validates it via `Authorization: Bearer <token>`. No user table, no signup/reset flow — one account by design.
